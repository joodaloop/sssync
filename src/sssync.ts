...
export class SSSync<Definitions extends EventDefinitions, Schemas extends TableSchemas> {
  ...
  async query(path: string, now = Date.now()): Promise<SyncResponse<Schemas>> {
    ...
    const stored = (await database.get(
      ...
      path
    )) as QueryCacheEntry<SyncResponse<Schemas>> | undefined
    if (stored) {
  ...
  private createSyncResponseSchema(
    tableSchemas: Schemas
  ): v.BaseSchema<unknown, SyncResponse<Schemas>, unknown> {
    const tableEntries = Object.entries(tableSchemas).map(([name, schema]) => [
      name,
      v.array(schema)
    ])
